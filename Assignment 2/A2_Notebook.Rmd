...
legend(
    "topright",
    inset=c(-0.75,0.4),
    legend = levels(factor(samples_type$infection)),
    fill = c("red", "blue"),
    xpd = NA
)
```
...
design <- model.matrix(~samples_type$treatment)
...
```{r count_sig_genes, eval=TRUE, warning=FALSE, message=FALSE}
...
qlf.cbd_vs_dmso <- edgeR::glmQLFTest(
    fit,
    coef = 'samples_type$treatmentDMSO'
)

# Extract the most significant genes
top_cbd_vs_dmso <- edgeR::topTags(qlf.cbd_vs_dmso, n = nrow(d))

# Count the number of significant genes (PValue & FDR) and write the counts in a kable table
n_sig_genes <- data.frame(
    PValue = length(which(top_cbd_vs_dmso$table$PValue < 0.05)),
    FDR = length(which(top_cbd_vs_dmso$table$FDR < 0.05))
)
...
heatmap_matrix <- as.matrix(filtered_data_matrix)

# Set the row names to be the gene names
rownames(heatmap_matrix) <- rownames(filtered_data_matrix)

# Set the column names to be the sample names
colnames(heatmap_matrix) <- colnames(filtered_data_matrix)

# Get the row names of the significantly differentially expressed genes
sig_genes <- rownames(qlf.cbd_vs_dmso$table[top_cbd_vs_dmso$table$FDR < 0.05, ])
