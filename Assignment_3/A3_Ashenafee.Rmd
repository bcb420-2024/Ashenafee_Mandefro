...
em_image_stringent <- file.path(
    em_output_dir,
    paste0(em_analysis_name, "_stringent_network.png")
)
...
knitr::include_graphics(
    paste0(
        "./output/", "em_results/", em_analysis_name, "_stringent_network.png"
    )
)
```
...
knitr::kable(
    data.frame(
        ...
        NumNodes = c(num_nodes_initial, num_nodes_stringent),
        NumEdges = c(num_edges_initial, num_edges_stringent)
    ),
...
To add more information to the visual, we will annotate the map using the `AutoAnnotate` app available in Cytoscape [@Kucera2016]. Specifically, the annotation will use the `annotate-clusterBoosted` method.

```{r annotate_enrichment_map, eval=TRUE, warning=FALSE, message=FALSE}
...
```{r show_initial_map, echo=FALSE, warning=FALSE, error=FALSE, out.width = "100%", fig.show='hold', fig.cap="Initial Enrichment Map of GSEA Results Comparing SARS-CoV-2-infected and Healthy ACE549 cells treated with CBD (p = 1.0, q = 0.05)"}
knitr::include_graphics(c("output/em_results/CBD_vs_DMSO_initial_network.png"))
```

```{r show_stringent_map, echo=FALSE, warning=FALSE, error=FALSE, out.width = "100%", fig.show='hold', fig.cap="Stringent Enrichment Map of GSEA Results Comparing SARS-CoV-2-infected and Healthy ACE549 cells treated with CBD (p = 1.0, q = 0.01)"}
knitr::include_graphics(c("output/em_results/CBD_vs_DMSO_stringent_network.png"))
```

**2. How many nodes and how many edges are in your enrichment map?**

In the *initial network* that was constructed with `p = 1.0` and `q = 0.05`, there are `r num_nodes_initial` nodes and `r num_edges_initial` edges. In the *stringent network* that was constructed with `p = 1.0` and `q = 0.01`, there are `r num_nodes_stringent` nodes and `r num_edges_stringent` edges.

**3. What thresholds did you use to create your enrichment map?**

To create the enrichment map, I used the following thresholds:

| Network Version | P-Value Threshold | Q-Value Threshold |
|-----------------|-------------------|-------------------|
| Initial         | 1.0               | 0.05              |
| Stringent       | 1.0               | 0.01              |
...
```{r show_annotated_map, echo=FALSE, warning=FALSE, error=FALSE, out.width = "100%", fig.show='hold', fig.cap="Annotated Enrichment Map of GSEA Results Comparing SARS-CoV-2-infected and Healthy ACE549 cells treated with CBD (p = 1.0, q = 0.01)"}
knitr::include_graphics(c("output/em_results/CBD_vs_DMSO_annotated_network.png"))
```

```{r show_annotated_theme_map, echo=FALSE, warning=FALSE, error=FALSE, out.width = "100%", fig.show='hold', fig.cap="Annotated Enrichment Map of GSEA Results Comparing SARS-CoV-2-infected and Healthy ACE549 cells treated with CBD (p = 1.0, q = 0.01)"}
knitr::include_graphics(c("output/em_results/CBD_vs_DMSO_annotated_theme_network.svg"))
```

**5. Create a publication ready figure.**

The figure below is in `.svg` format to allow for easy editing and scaling, which is often used for publication-quality networks.
